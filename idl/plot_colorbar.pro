...
if not keyword_set(block) then begin
  if keyword_set(other) then begin
    xyouts, xx-0.03, ynorm[1]+0.05, other, /normal, color = color, charsize = 1, charthick = 1
     ...
     xyouts, xx-0.02, ynorm[1]+0.05,  '!3'+type, /normal, color = color, $
		charsize = textcharsize, charthick = 1
     if keyword_set(type1) then $
       xyouts, xx-0.025, ynorm[1]+0.04,  '!3'+type1, /normal, color = color, $
		charsize = textcharsize, charthick = 1
       ;xyouts, xx-0.02, ynorm[1]+0.055,  '!3'+type+' Range', /normal, color = color, charsize = 1.3, charthick = 1
       xyouts, xx-0.02, ynorm[1]+0.01, '('+unit+')', /normal, color = color, $
		charsize = textcharsize, charthick = 1
  endelse
endif else begin
...
xyouts, xx+0.034, ynorm[0], strcompress(string(format='(F6.1)',z0), /remove_all), /normal, $
		color = color, charsize = rangecharsize;, charthick = 1
xyouts, xx+0.034, ynorm[1]-0.01, strcompress(string(format='(F6.1)',z1), /remove_all), /normal, $
		color = color, charsize = rangecharsize;, charthick = 1
