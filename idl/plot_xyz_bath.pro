pro plot_xyz_bath, data_arr, min_z=min_z, max_z=max_z, $
	plot_range=plot_range, title=title, win = win
...
window, 0, xsize=800, ysize=800, color = -1
...
for i = 0, n_arr-1 do begin
    indx = where( ( (*data_arr[i]).depth[0] + (*data_arr[i]).elevation[0]/100.) lt 0.0 )
...
!p.region=[0,0,1,1]
plot_colorbar, [min_z, max_z], "!3 NAVD88 GEOID !3", "!3 meters !3"
