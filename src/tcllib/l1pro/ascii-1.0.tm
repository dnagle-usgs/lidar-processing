...
snit::widget ::l1pro::ascii::gui {
   ...
   constructor args {
      ...
      ttk::label $w.previewlbl \
         -text "Preview of import (click on column headings to re-assign):"
      preview $w.preview
      ...
      if {[llength $lastvals]} {
         ...
         $preview configure -columnmappings [dict get $lastvals columnmapping]
      }
   }
   ...
   method build_pane_import w {
      ...
      ttk::button $w.load -text "Load" \
         -command [mymethod do_import]
      ttk::button $w.dismiss -text "Dismiss" \
         -command [mymethod dismiss]
   ...
   method build_pane_convert w {
      ...
      ttk::entry $w.file -state readonly \
         -textvariable [myvar destfn]
      ttk::button $w.filebrowse -text "Browse..." \
         -command [mymethod select_destfn]

      ttk::frame $w.btns
      ttk::button $w.convert -text "Convert" \
         -command [mymethod do_convert]
      ttk::button $w.dismiss -text "Dismiss" \
         -command [mymethod dismiss]
   ...
   method build_pane_batch w {
      ...
      ttk::entry $w.indir -state readonly \
         -textvariable [myvar srcdir]
      ...
      ttk::entry $w.outdir -state readonly \
         -textvariable [myvar outdir]
      ...
      ::tooltip::tooltip $w.vprefix \
         "This field provides a prefix that will be prepended to each file's\
         \nvariable name."
      ::tooltip::tooltip $w.vmid \
         "Each file will receive a variable name derived from the file's name.\
         \nIf possible, that name will be a clean form of the tile name encoded\
         \nin the filename. Otherwise, the filename without its extension is\
         \nused."
      ::tooltip::tooltip $w.vsuffix \
         "This field provides a suffix that will be appended to each file's\
         \nvariable name."
   ...
   method load_sample {} {
      if {$filename eq "" || ![file isfile $filename] || ![file readable $filename]} {
         return
      }
