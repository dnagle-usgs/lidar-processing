...
#
# Each of the above commands will launch the embedded window GUI for cf
...
proc ::eaarl::cfconf::plotcmd {window args} {
    set gui [config $window {*}$args]
    return [$gui plotcmd]
}

proc ::eaarl::cfconf::plot {window args} {
    set gui [config $window {*}$args]
    $gui plot
    return $gui
}

proc ::eaarl::cfconf::config {window args} {
    set gui [namespace current]::window_$window
    if {[info commands $gui] ne ""} {
        $gui configure {*}$args
    } else {
        ::eaarl::cfconf::embed $gui {*}$args -window $window
    }
    return $gui
}

snit::type ::eaarl::cfconf::embed {
    option -window -readonly 1 -default 28 -configuremethod SetOpt
    ...
    method Gui_sync {f} {
        if {$win_width > 600} {
            $sync build_gui $f.fraSync -exclude cf -layout onecol
        } else {
            $sync build_gui $f.fraSync -exclude cf -layout wrappack
        }
    ...
    method Gui_settings_all {f} {
        ...
        ttk::label $f.lblInitSD -text "InitSD: "
        ttk::spinbox $f.spnInitSD \
                -from 0 -to 10 -increment .1 \
                -width 4
        ::mixin::revertable $f.spnInitSD \
                -command [list $f.spnInitSD apply] \
                -valuetype number
        ...
        if {$win_width > 600} {
            ...
            grid configure $f.lblSmooth $f.lblThresh $f.lblCurve $f.lblInitSD \
                    -sticky w
            grid configure $f.spnSmooth $f.spnThresh $f.cboCurve $f.spnInitSD \
                    -sticky ew
            grid columnconfigure $f 1 -weight 1
        } else {
            foreach item {Smooth Thresh InitSD} {
            ...
            foreach item {Curve} {
                lower [ttk::frame $f.fra$item]
                pack $f.lbl$item $f.cbo$item -in $f.fra$item -side left
                wrappack $f.fra$item -padx 2 -pady 1
            }
        }

        lappend controls $f.spnSmooth
        dict set wantsetting $f.spnThresh thresh
        dict set wantsetting $f.spnInitSD initsd
        dict set wantsetting $f.spnSmooth smoothwf
        dict set wantsetting $f.cboCurve  curve
    }
    ...
    method ProfileAdd {} {
        if {
            ...
            exp_send "cfconf, profile_add, \"$options(-group)\",\
                    \"$text\";\r"
        }
    }

    method ProfileDel {} {
        exp_send "cfconf, profile_del, \"$options(-group)\",\
                \"$::eaarl::cfconf::active_profile($options(-group))\";\r"
    }

    method ProfileRename {} {
        set old $::eaarl::cfconf::active_profile($options(-group))
        if {
            [::getstring::tk_getString $window.gs new \
                    "What would you like to rename \"$old\" to?"]
        } {
            if {$old ne $new} {
                exp_send "cfconf, profile_rename, \"$options(-group)\",\
                        \"$old\", \"$new\";\r"
            }
        }
    }
    ...
    method FileLoad {} {
        set fn [tk_getOpenFile \
                ...
                -filetypes {
                    {{Curve Fitting configuration files} {.cfconf}}
                    {{All files} {*}}
                }]
        if {$fn ne ""} {
            exp_send "cfconf, read, \"$fn\"; "
            $self plot
        }
    }
    ...
    method UpdateTitle {} {
        wm title $window "Window $options(-window) - CF -\
    ...
    method SetKey {key old new} {
        exp_send "cfconf, set, \"$options(-group)\", \"$key\", \"$new\"; "
        $self plot
        return -code error
    }

    method SetKeyVar {key} {
        set val $::eaarl::cfconf::settings(${options(-group)},${key})
        exp_send "cfconf, set, \"$options(-group)\", \"$key\", \"$val\"; "
        $self plot
    }

    method SetProfile {old new} {
        exp_send "cfconf, profile_select, \"$options(-group)\", \"$new\"; "
        $self plot
        return -code error
    }
    ...
    method prompt_groups {} {
        exp_send "cfconf, prompt_groups, \"cfconf\", \"cfconf\", $options(-window);\r"
    }
}
