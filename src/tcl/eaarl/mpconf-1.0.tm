...
#
# Each of the above commands will launch the embedded window GUI for mp
...
proc ::eaarl::mpconf::plotcmd {window args} {
    set gui [config $window {*}$args]
    return [$gui plotcmd]
}

proc ::eaarl::mpconf::plot {window args} {
    set gui [config $window {*}$args]
    $gui plot
    return $gui
}

proc ::eaarl::mpconf::config {window args} {
    set gui [namespace current]::window_$window
    if {[info commands $gui] ne ""} {
        $gui configure {*}$args
    } else {
        ::eaarl::mpconf::embed $gui {*}$args -window $window
    }
    return $gui
}
...
snit::type ::eaarl::mpconf::embed {
    ...
    method Gui_sync {f} {
        if {$win_width > 600} {
            $sync build_gui $f.fraSync -exclude multipeak -layout onecol
        } else {
            $sync build_gui $f.fraSync -exclude multipeak -layout wrappack
        }
    ...
    method ProfileAdd {} {
        if {
            ...
            exp_send "mpconf, profile_add, \"$options(-group)\",\
                    \"$text\";\r"
        }
    }

    method ProfileDel {} {
        exp_send "mpconf, profile_del, \"$options(-group)\",\
                \"$::eaarl::mpconf::active_profile($options(-group))\";\r"
    }

    method ProfileRename {} {
        set old $::eaarl::mpconf::active_profile($options(-group))
        if {
            [::getstring::tk_getString $window.gs new \
                    "What would you like to rename \"$old\" to?"]
        } {
            if {$old ne $new} {
                exp_send "mpconf, profile_rename, \"$options(-group)\",\
                        \"$old\", \"$new\";\r"
            }
        }
    }
    ...
    method SetKey {key old new} {
        exp_send "mpconf, set, \"$options(-group)\", \"$key\", \"$new\"; "
        $self plot
        return -code error
    }

    method SetKeyVar {key} {
        set val $::eaarl::mpconf::settings(${options(-group)},${key})
        exp_send "mpconf, set, \"$options(-group)\", \"$key\", \"$val\"; "
        $self plot
    }

    method SetProfile {old new} {
        exp_send "mpconf, profile_select, \"$options(-group)\", \"$new\"; "
        $self plot
        return -code error
    }
    ...
    method prompt_groups {} {
        exp_send "mpconf, prompt_groups, \"mpconf\", \"mpconf\", $options(-window);\r"
    }
}
