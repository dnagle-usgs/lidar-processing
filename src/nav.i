...
/* DOCUMENT pl_fp(fp, win=, color=, width=, labels=, skip=)
  ...
  Parameter:
    fp: A flight plan variable in FB struct.
  Options:
  ...
  The input FP may be an array of FB structs, in which case each will be
...
*/
  ...
  // Initialize fb using info in the first three lines which are usually header
  ...
  fb.name = strtrim(longest_common_prefix(*cols(2)), 2, blank="-");
...
/* DOCUMENT pip_fp(fp, ply=, win=)
  ...
  fp=     The input fp variable.  This is generated by sdist().
    It must be of type FB.  If fp is not set, the user
...
*/
  ...
  km = sum(segdist);
  segsecs = sum(segdist*1000./fb.msec);
  blocksecs = segsecs+(fb.ssturn*numberof(segdist));
...
/* DOCUMENT write_globalmapper_fp(fp, outfile=, out_utm=)
  ...
  Parameters:
    input: The flight plan array (in FB struct) or the name of a flight
      planning file.
  Options:
