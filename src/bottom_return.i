...
func make_ebs_from_edf(dirname, fname=, data=, write_ebs=) {
...
extern edb, soe_day_start, data_path, nostats;
 ...
 for (df=1;df<=numberof(fn_all);df++) {
  ...
  if (!is_array(data)) {
   // read the edf file
   data_ptr = read_yfile(dirname, fname_arr=fname);
   data = *data_ptr(1);
  }
  ...
  for (i=1;i<=numberof(idx)-1;i++){
	...
	ebsarr(ecount) = bret;
        if (j%100 == 0) write, format="%d of %d completed for this day\r", j,numberof(daydata);
    }
  }
...
func load_this_edb(soe_ds) {
  ...
  */
  ...
  if (soe_ds >= 1028246400) data_path = path_aug02+dates_aug02(idx)+"/";
