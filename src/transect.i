...
/* DOCUMENT extract_transect_info(tlst, fs, &coords, &segtimes, rtn=)
  This function saves relevant transect information.
  INPUT:
    tlst: returned from the mtransect function. list of indices of the fs array
      that fall along the transect.
    ...
    rtn = defaults to 0.
        ...
        2 submerged topo
*/
// Original Amar Nayegandhi Nov 2006
...
/* DOCUMENT append_transect_list(tlst, fs, rtn=)
  This function appends the transect coordinates and time of day to the global
  variables coords_all and transect_all array.
  INPUT:
    tlst: returned from the mtransect function. list of indices of the fs array
      that fall along the transect.
    fs: the original data array (can be of type FS, VEG, or GEO)
...
/* DOCUMENT reprocess_data_along_transect(new_pnav, outdir=, ofname_tag=, rtn=,
   w=)
  This function reprocesses data along the transects defined by the transect_all
  array.

  INPUT:
  new_pnav : the new pnav data array.  The global variables gga and pnav will
    be assigned to this new_pnav
  outdir = String.  The output directory where the pbd files will be written
    to.
  ofname_tag = String.  Define tag name to the output filename that will help
    differentiate between data processed using different trajectories.  Usually
    name_tag will include the base station names used to processed the
    trajectory new_pnav. e.g. name_tag = "kwal_hg63" indicates trajectory
    processed using the Wallops Island base and the Hangar base station.
  rtn = defaults to 0.
    Select return type where:
      0 first return
      1 veg last return
...
*/
// Original Amar Nayegandhi Jan 2006
