	...
	ply = array(float, 2, 1)				// array that contains polygon vertices
 	...
 	plmk, ply(2, 0), ply(1, 0), marker = 4, msize = .5, width = 10
  	...
  	{		
    	...
    	plmk, ply(2, 0), ply(1, 0), marker = 4, msize = .5, width = 10
    	plg, [ply(2,-1), ply(2,0)],  [ply(1,-1), ply(1,0)], marks = 0; //connect current and previous						
  	}
  	plg, [ply(2,1), ply(2,0)],  [ply(1,1), ply(1,0)], marks = 0;   	//connect first and last vertex
  	return ply								
}
...
*/
{
	...
	area = ((x > xl) & (x < xh) & (y > yl) & (y < yh))	// area bounded by the box
