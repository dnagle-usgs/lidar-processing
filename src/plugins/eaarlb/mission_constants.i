...
*/
  ...
  if(ytk) {
    ...
    cmd = swrite(format="::eaarlb::settings::ops_conf::view {%s}", json);
    if(!is_void(name))
...
/* DOCUMENT eaarl_ops_conf_gui_init
  Glue for ::eaarlb::settings::ops_conf::gui.
*/
  extern ops_conf;
  fields = get_members(ops_conf);
  fields = strjoin(fields, " ");
  tkcmd, swrite(format="::eaarlb::settings::ops_conf::gui_init {%s}", fields);
}
