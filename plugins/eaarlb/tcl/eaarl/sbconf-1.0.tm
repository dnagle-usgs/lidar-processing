...
snit::type ::eaarl::sbconf::embed {
    ...
    method prompt_groups {} {
        exp_send "sbconf, prompt_groups, $options(-window);\r"
    }
}

snit::widgetadaptor ::eaarl::sbconf::prompt_groups {
    option -window -1
    ...
    method apply {} {
        ...
        set cmd "sbconf, groups, save([join $chunks ,]); "
        exp_send "$cmd\r"
        if {$options(-window) >= 0} {
            after 1000 ::eaarl::sbconf::plot $options(-window)
        }
        return done
    }
}
