...
//  *                                                     *
//  V                                                     V
func eaarl_cf_rx_channel_shallow(pulses) {
/* DOCUMENT eaarl_cf_rx_channel_shallow(pulses)
...
*/
  ...
  eaarl_cf_rx_channel, pulses;
}

func hook_eaarl_cf_shallow(env) {
/* DOCUMENT hook_cf_shallow(env)
  This hook is attached to process_cf_funcs to enable shallow bathy hacks.
*/
  save, env, cf_rx=eaarl_cf_rx_channel_shallow;
  return env;
}

func eaarl_cf_shallow(action) {
/* DOCUMENT
  eaarl_cf_shallow, "enable";
  eaarl_cf_shallow, "disable";
  eaarl_cf_shallow, "status";

  Enables, disables, or gives the status of the shallow bathy hacks for veg
  processing.
*/
  if(action == "enable") {
    hook_add, "process_cf_funcs", "hook_eaarl_cf_shallow";
    write, "shallow bathy hacks ENABLED";
  } else if(action == "disable") {
    hook_remove, "process_cf_funcs", "hook_eaarl_cf_shallow";
    write, "shallow bathy hacks DISABLED";
  } else if(action == "status") {
    hooks = hook_query("process_cf_funcs");
    if(anyof(hooks == "hook_eaarl_cf_shallow")) {
